...
- **레벨 1회 발화(ATH 구간 기준)**: 한 매수/매도 레벨은 같은 ATH 구간에서 1회만 알림한다.
  상태(active_levels·level_last_alert)는 **매 거래일이 아니라 ATH가 위로 갱신될 때만** 리셋되어
  재무장된다 → 가격이 -10% 아래로 갔다가 회복 후 다시 -10%를 재돌파해도 중복 알림이 가지 않음.
  ATH가 새 전고점으로 확정(위로 갱신)되면 그 새 기준에서 모든 레벨이 다시 1회씩 발화 가능.
  최초 등록 시점엔 이미 도달한 레벨을 baseline으로 억제(가입 전 하락분 스팸 방지), 이후 더 깊은
  레벨부터 알림. 구간 유지 재알림(redrawdown_repeat_interval)은 별개 옵션으로, 레벨에 머무는 동안
  N분 간격 재알림. 백테스트(web/src/lib/signals.js)도 동일하게 "ATH 구간당 레벨 1회"로 발화한다.
